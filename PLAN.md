...
- [x] Where to launch for initial validation—**decided Sep 16, 2026, see section 21 below (soft launch r/mcp + MCP/Anthropic Discord first, hard launch Show HN after; both gated on the site redesign). Tracked in Plane: MCPFO-15.**
...
## 21. Launch channel decision: soft→hard ladder, both gated on the site redesign (Sep 16, 2026)

Closes MCPFO-15 (the last open launch question in §5). The repo, npm (`klaridian`, unscoped), and PyPI (`klaridian`) are all already public — so "launch" is no longer a future flip; the remaining decision is purely *where and in what order to point people for initial validation*.

**Decision: a soft→hard ladder, not a single channel.** Going straight to Show HN is one shot at a high-variance, adversarial audience — wasteful if the v0 isn't polished, and it burns the one big moment. Instead:

1. **Soft launch — r/mcp + the MCP/Anthropic Discord community.** An audience that already understands MCP (no need to explain the category), tolerant, gives feedback to sand off rough edges, and yields the first real users and 1-2 proof points for the hero.
2. **Hard launch — Show HN**, only after step 1, with the site polished and a couple of proof points in hand. There the high variance works in our favour.
3. **X** stays as amplification, dependent on niche audience — weak signal on its own.

**Why this reframe does NOT contradict §18 (audience = agents).** Two different questions that are easy to conflate: the *adopter* long-term is the agent (served by product features — MCPFO-101 registry-ready output — and passive discovery — llms.txt, registries), which is slow and hard to measure early. The *initial-validation channel* is human — fast, measurable signal (comments, critique, 24h star velocity) on whether to keep investing. §18 sharpens who the product is for; it does not remove the human validation moment, which is simply the last rung of this ladder.

**Hard gates (both rungs), decided this session:**
- **The site redesign (MCPFO-100) blocks BOTH rungs, not just the hard launch.** "Published" ≠ "ready to point people at." The current site is not good enough to send even a friendly r/mcp audience to. MCPFO-100 promoted to `high` and reframed as a launch blocker.
- **The orphan-commit leak (MCPFO-60) blocks the HARD launch specifically.** HN brings adversarial eyes and competitors — precisely the audience that would SHA-probe the history and publish "their competitive analysis is exposed." The fix is a 2-minute private→public toggle (MCPFO-60 Option A, ~0 stars to lose pre-launch), done the eve of the Show HN. It does NOT block the soft launch: r/mcp/Discord won't SHA-probe, and the residual risk was already consciously accepted (MCPFO-60, Sep 8).
