...
**Honest limitation found and documented, not hidden, while validating #3/#4 (an instance of the same "unpatchable third-party output" class of finding as the section 29 `streamable-http` bug):** `openapi-mcp-generator`'s own `generateMcpServer()` call prints a substantial amount of its own progress text directly to stderr (`"Parsing OpenAPI spec..."`, `"Generating server code..."`, per-file `"-> Created ..."` lines, etc.) — this is **not mcpforge's own output** and mcpforge has no supported way to silence it (no verbosity option exposed by `openapi-mcp-generator`'s API). Consequence: `--quiet` and even `--json` mode still see this third-party text land on stderr during generation. This does **not** break `--json`'s core guarantee — confirmed directly that **stdout stays exactly one valid JSON value** regardless (`openapi-mcp-generator` only writes to stderr, never stdout, matching the same stdio-safety property already verified for its generated servers in sections 12/20) — but a caller doing `2>&1` redirection or expecting fully silent `--quiet` operation will still see this noise. Documented here rather than silently claiming `--quiet`/`--json` fully control all program output.
