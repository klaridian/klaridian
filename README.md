...
- **MCP Registry + versioning:** `--registry-name io.github.you/server` sets the reverse-DNS name in `server.json` + `package.json`'s `mcpName`, and marks the project publishable (`package.json` is not `private` and `server.json` declares a matching npm package entry). The server's **version is derived**, not hardcoded: `--server-version <x.y.z>` wins, else the spec's `info.version` if it's valid SemVer, else `1.0.0` — stamped identically into the runtime, `package.json`, and `server.json`. Non-SemVer fails loudly (the registry marks any unparseable version "latest"). Without `--registry-name` the project stays private and `server.json` declares no package, so the two files never contradict each other. See [Publishing to the MCP Registry](packages/site/content/docs/how-to/mcp-registry.mdx) and ARCHITECTURE.md sections 96–97.
