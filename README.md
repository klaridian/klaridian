...
- **Deploy:** `klaridian deploy <dir> --target docker` emits a portable `Dockerfile` + `.dockerignore` for a generated streamable-http server (TypeScript or Python), following an emit + shell-out model—klaridian writes the platform's native config, you hand it to the platform's CLI. The generated server is deploy-ready: it reads `PORT` and `KLARIDIAN_ALLOWED_HOSTS` from the environment so one build runs unchanged behind a public host (see [Deploy](packages/site/content/docs/how-to/deploy.mdx) and ARCHITECTURE.md sections 78–80). More targets (Cloudflare Workers, Fly) are planned on the same model.
