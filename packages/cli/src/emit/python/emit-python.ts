...
import { emitServerJson } from "../emit-project-files.js";
...
function emitReadme(opts: EmitOptions): string {
  ...
  const runLine =
    transport === "streamable-http"
      ...
      : `python server.py`;
  // MCPFO-101: PyPI package-ownership proof for the official MCP Registry. The
  // registry validator fetches the published package's description (which PyPI
  // builds from this README) and requires the literal `mcp-name: <server.json
  // name>` to appear in it. It's kept in an HTML comment so it doesn't clutter
  // the rendered README, and emitted only for a publishable project (a
  // --registry-name was given) so the name matches server.json byte-for-byte.
  // (The npm target proves ownership via package.json `mcpName` instead.)
  const mcpNameProof = opts.publishable && opts.registryName ? `\n<!-- mcp-name: ${opts.registryName} -->\n` : "";
...
export function emitPythonProject(opts: EmitOptions): EmittedProject {
  ...
  const files: EmittedProject = {
    ...
    // (when publishable) points at PyPI with a uvx runtimeHint; ownership is
    // proved by the `mcp-name:` line emitReadme() puts in README.md.
