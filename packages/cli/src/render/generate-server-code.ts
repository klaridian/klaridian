...
function generateToolBlock(tool: ToolDefinition, wrapFunctionName?: string): string {
...
export function generateServerSource(
  ...
  options: { serverName: string; version: string; plugins?: ObservabilityPlugin[] }
): string {
  ...
  const toolBlocks = tools.map((t) => generateToolBlock(t, wrapFunctionName)).join("\n");
