...
| Flag | What it does |
| --- | --- |
| `--target <target>` | Which target to emit for: `docker` or `cloudflare`. |
...
A Fly target (a one-command container deploy over the same Dockerfile) is planned on top of this same emit + shell-out model.
