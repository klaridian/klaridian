...
export function getSecurityHelpersFileContent(): string {
  return `// GENERATED by klaridian (security hardening) — vendored helper module.
// See ARCHITECTURE.md section 30 in the klaridian repo for why these exist:
 ...
 * KLARIDIAN_RATE_LIMIT_PER_MINUTE; set to "0" to disable entirely (e.g. for
...
const RATE_LIMIT_PER_MINUTE = (() => {
  const raw = process.env.KLARIDIAN_RATE_LIMIT_PER_MINUTE;
 ...
 * Configurable via KLARIDIAN_MAX_OUTPUT_CHARS. Guards against a single tool
...
const MAX_OUTPUT_CHARS = (() => {
  const raw = process.env.KLARIDIAN_MAX_OUTPUT_CHARS;
