...
- **Deploy:** `klaridian deploy <dir> --target docker` emits a portable `Dockerfile` + `.dockerignore` (TypeScript or Python), and `--target cloudflare` emits a Cloudflare Worker + `wrangler.toml` (TypeScript, edge) — both following an emit + shell-out model, where klaridian writes the platform's native config and you hand it to the platform's CLI. The generated server is deploy-ready: it reads `PORT` and `KLARIDIAN_ALLOWED_HOSTS` from the environment so one build runs unchanged behind a public host (see [Deploy](packages/site/content/docs/how-to/deploy.mdx) and ARCHITECTURE.md sections 78–81). A Fly target is planned on the same model.
