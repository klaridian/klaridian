...
- **Every non-trivial change should be validated end to end**, not just unit-tested. This project's own tests do real `npm install`/`tsc build`/spawn-and-drive-over-stdio for the TypeScript CLI—because the riskiest failure modes only show up at that level.
