...
- Generated content (API reference tables, changelogs assembled from commit messages, etc.) is exempt where the source data itself doesn't follow prose conventions—for example, `packages/site/content/docs/reference/cli-reference.mdx`, produced verbatim from `klaridian generate`'s real flag descriptions by `packages/cli/scripts/generate-cli-docs.mjs` (see AGENTS.md's working agreements). Fix wording by editing the flag's `.option()` description in `packages/cli/src/commands/generate.ts` and regenerating, not by hand-editing the `.mdx`.
